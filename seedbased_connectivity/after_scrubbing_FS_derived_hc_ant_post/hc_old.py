...
def create_transform_hc(name='transform_hc'):
    ...
    extract_hc.inputs.match=[17] #17=left, 53=right   
...
#    #resample to functional space (128x128x128 (2x2x2) or in LIFE (85x85x85 (3x3x3mm))
    ...
    def iterable_list(working_dir):
        filename= [working_dir + '/hc_connec_thr099/transform_hc/posterior_hc.nii.gz', working_dir + '/hc_connec_thr099/transform_hc/anterior_hc.nii.gz']
        print filename
        return filename
    
    it_list=Node(util.Function(input_names=['working_dir'],output_names=['filename'],function=iterable_list),
                 name='it_list')
    ...
    transform_hc.connect([
                      ...
                      (inputnode, it_list, [('working_dir','working_dir'),
                                            ('fs_subject_id', 'subject_id')
                                             ]),  
