...
def create_topup_coreg_pipeline(name='fmap_coreg'):
    ...
    topup_prep.inputs.encoding_file='/home/raid1/fbeyer/Documents/Scripts/RSV_scripts/3Tpreprocessing/functional_preprocessing/topup-version/aquisition_params_RSV.txt'
    ...
    #would be used if directly applying topup results to data, but we want to do the combined registration with anatomical  
    #apply topup
    #apply_topup=Node(fsl.ApplyTOPUP(), name='apply_topup')
    
    #only needed when using Fieldmap-measurement and  PrepareFieldmap() command
