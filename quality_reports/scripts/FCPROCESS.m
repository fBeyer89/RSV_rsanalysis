function [allends switches] = FCPROCESS(datafile,tdir,tmasktype,cleanup,roilist,varargin)
...
for i=1:size(rois,1)
    roimasks(:,i)=read_4dfpimg(rois{i,1});
end
...
for i=1:numsubs
    ...
    for j=1:length(FSfile.dir)
        if isequal(FSfile.vc{j,1},QC(i).vcnum)
            QC(i).fsdir=FSfile.dir{j,1};
            found=found+1;
        end
    end
    if found==1
...
for i=1:numsubs
    ...
    QC(i).restruns=str2num(tempruns);
    
    % cycle through each BOLD run
    for j=1:numel(QC(i).restruns)
        
        % set original data
        boldmat{i,j} = [ df.filepath{i,1} '/' QC(i).vcnum '/bold' num2str( QC(i).restruns(j)) '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_xr3d.mat' ];
        boldimg{i,j} = [ df.filepath{i,1} '/' QC(i).vcnum '/bold' num2str( QC(i).restruns(j)) '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_xr3d_333.4dfp.img' ];
        boldifh{i,j} = [ df.filepath{i,1} '/' QC(i).vcnum '/bold' num2str( QC(i).restruns(j)) '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_xr3d_333.4dfp.ifh' ];
    ...
    anataveimg{i,1} = [ df.filepath{i,1} '/' QC(i).vcnum '/atlas/' QC(i).vcnum '_anat_ave_t88_333.4dfp.img'];
    anataveifh{i,1} = [ df.filepath{i,1} '/' QC(i).vcnum '/atlas/' QC(i).vcnum '_anat_ave_t88_333.4dfp.ifh'];
    
    mprimg{i,1}=ls([ df.filepath{i,1} '/' QC(i).vcnum '/atlas/' QC(i).vcnum '_mpr_n*_333_t88.4dfp.img']);
    mprifh{i,1}=ls([ df.filepath{i,1} '/' QC(i).vcnum '/atlas/' QC(i).vcnum '_mpr_n*_333_t88.4dfp.ifh']);
    mprimg{i,1}(end)=[];
    mprifh{i,1}(end)=[];
...
for i=1:numsubs
    ...
    tanataveimg{i,1} = [ QC(i).subatlasdir '/' QC(i).vcnum '_anat_ave_t88_333.4dfp.img'];
    tanataveifh{i,1} = [ QC(i).subatlasdir '/' QC(i).vcnum '_anat_ave_t88_333.4dfp.ifh'];
    system([ 'ln -s ' anataveimg{i,1} ' ' tanataveimg{i,1}]);
    system([ 'ln -s ' anataveifh{i,1} ' ' tanataveifh{i,1}]);
    
    tmprimg{i,1} = [ QC(i).subatlasdir '/' QC(i).vcnum '_mpr_333_t88.4dfp.img'];
    tmprifh{i,1} = [ QC(i).subatlasdir '/' QC(i).vcnum '_mpr_333_t88.4dfp.ifh'];
    system([ 'ln -s ' mprimg{i,1} ' ' tmprimg{i,1}]);
    system([ 'ln -s ' mprifh{i,1} ' ' tmprifh{i,1}]);
    
    system([ 'cp ' df.prmfile{i,1} ' ' QC(i).subdir '/fc.params' ]);
    
    QC(i).MPRAGE=read_4dfpimg(mprimg{i,1});
    QC(i).ANATAV=read_4dfpimg(anataveimg{i,1});
    
    % cycle through each BOLD run
    for j=1:numel(QC(i).restruns)
        
        % prepare and enter targetsubbolddir
        QC(i).subbolddir{j}=[QC(i).subdir '/bold' num2str(QC(i).restruns(j))];
        ...
        tboldmat{i,j} = [ QC(i).subbolddir{j} '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_xr3d.mat'];
        tboldimg{i,j} = [ QC(i).subbolddir{j} '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_xr3d_333.4dfp.img'];
        tboldifh{i,j} = [ QC(i).subbolddir{j} '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_xr3d_333.4dfp.ifh'];
...
for i=1:numsubs
    for j=1:numel(QC(i).restruns)
        LASTIMG{i,j,stage}=[ QC(i).subbolddir{j} '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_xr3d_333' ];
    end
    cd(QC(i).subdir);
...
for i=1:numsubs
    ...
    system(commands);
    pause(5);
    QC(i).DFNDVOXELS=read_4dfpimg([LASTCONC{stage} '_dfnd.4dfp.img']);
end
...
if switches.doregression
    ...
    switch switches.regressiontype
        case 0 % classic WashU seeds in WM and V
            ...
            for i=1:numsubs
                QC(i).GLMmaskfile=['/home/usr/fidl/lib/glm_atlas_mask_' voxdim '.4dfp.img'];
                QC(i).WMmaskfile=['/home/usr/fidl/lib/small_WM.4dfp.img'];
                QC(i).CSFmaskfile=['/home/usr/fidl/lib/711-2B_lat_vent_333.4dfp.img'];
            end
            ...
            for i=1:numsubs
                ...
                tmpmask=read_4dfpimg(QC(i).WMmaskfile);
                tmpmask=tmpmask & QC(i).DFNDVOXELS;
                if nnz(tmpmask)==0
                    fprintf('%s is empty!\n',QC(i).WMmaskfile);
                    needtostop=1;
                ...
                tmpmask=read_4dfpimg(QC(i).CSFmaskfile);
                tmpmask=tmpmask & QC(i).DFNDVOXELS;
                if nnz(tmpmask)==0
                    fprintf('%s is empty!\n',QC(i).CSFmaskfile);
                    needtostop=1;
        ...
        case {1,9} % freesurfer masks of WM and V
            
            % set basic names
            for i=1:numsubs
                QC(i).GLMmaskfile=['/home/usr/fidl/lib/glm_atlas_mask_' voxdim '.4dfp.img'];
                QC(i).WMmaskfile=[ QC(i).fsdir '/' QC(i).vcnum '/nusmask/aparc+aseg_cerebralwm_ero' num2str(switches.WMero) '_mask_' voxdim '.4dfp.img'];
                QC(i).CSFmaskfile=[ QC(i).fsdir '/' QC(i).vcnum '/nusmask/aparc+aseg_CSF_ero' num2str(switches.CSFero) '_mask_' voxdim '.4dfp.img'];
                QC(i).WBmaskfile=[ QC(i).fsdir '/' QC(i).vcnum '/nusmask/aparc+aseg_brainmask_mask_333.4dfp.img'];
                QC(i).GREYmaskfile=[ QC(i).fsdir '/' QC(i).vcnum '/nusmask/aseg_GM_mask_' voxdim '.4dfp.img'];
            end
            ...
            for i=1:numsubs
                ...
                if ~exist([ QC(i).fsdir '/' QC(i).vcnum '/' ])
            ...
            for i=1:numsubs
                ...
                tmpmask=read_4dfpimg(QC(i).WBmaskfile);
                tmpmask=tmpmask & QC(i).DFNDVOXELS;
                if nnz(tmpmask)==0
                    fprintf('%s is empty!\n',QC(i).WBmaskfile);
                    needtostop=1;
                ...
                tmpmask=read_4dfpimg(QC(i).GREYmaskfile);
                tmpmask=tmpmask & QC(i).DFNDVOXELS;
                if nnz(tmpmask)==0
                    fprintf('%s is empty!\n',QC(i).GREYmaskfile);
                    needtostop=1;
                ...
                while nnz(tmpmask)==0
                    ...
                    QC(i).WMmaskfile = [ QC(i).fsdir '/' QC(i).vcnum '/nusmask/aparc+aseg_cerebralwm_ero' num2str(tmpWMero) '_mask_' voxdim '.4dfp.img'];
                    tmpmask=read_4dfpimg(QC(i).WMmaskfile);
                    tmpmask=tmpmask & QC(i).DFNDVOXELS;
                end
                QC(i).WMMASK=~~tmpmask;
                ...
                while nnz(tmpmask)==0
                    ...
                    QC(i).CSFmaskfile = [ QC(i).fsdir '/' QC(i).vcnum '/nusmask/aparc+aseg_cerebralwm_ero' num2str(tmpCSFero) '_mask_' voxdim '.4dfp.img'];
                    tmpmask=read_4dfpimg(QC(i).CSFmaskfile);
                end
                QC(i).CSFMASK=~~tmpmask;
    ...
    if switches.doregression
        switch switches.regressiontype
            case 0
                ...
                write_4dfpifh([ QC(i).subseeddir '/usedseeds.4dfp.img'],size(QC(i).seedhold,2),'bigendian');
...
for i=1:numsubs
    for j=1:numel(QC(i).restruns)
        ...
        cd(QC(i).subbolddir{j});
        fprintf('CALCULATING MOTION\t%d\t%s\trun%d\n',i,QC(i).vcnum,QC(i).restruns(j));
        
        % obtain realignment parameters
        system([ 'mat2dat -R -D -L ' tboldmat{i,j} ' >/dev/null' ]);
        pause(5);
        
        rdatfile{i,j}= [QC(i).subbolddir{j} '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_xr3d.rdat' ];
        ddatfile{i,j}= [QC(i).subbolddir{j} '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_xr3d.ddat' ];
...
for f=1:numsubs
    ...
    if switches.doregression
        ...
        write_4dfpimg(zb,'beta.4dfp.img','bigendian');
        write_4dfpifh('beta.4dfp.img',size(zb,2),'bigendian');
...
%     end
    ...
    if switches.doblur
        ...
        for j=1:numel(QC(i).restruns)
            temprun=tempimg(:,QC(i).runborders(j,2):QC(i).runborders(j,3));
            write_4dfpimg(temprun,[ LASTIMG{i,j,stage-1} '.4dfp.img'],'bigendian');
            write_4dfpifh([ LASTIMG{i,j,stage-1} '.4dfp.img'],size(temprun,2),'bigendian');
        end
    ...
    cd(QC(i).subdir);
    write_4dfpimg(tempimg,[ QC(i).vcnum '_' allends '.4dfp.img'],'bigendian');
    write_4dfpifh([ QC(i).vcnum '_' allends '.4dfp.img'],size(tempimg,2),'bigendian');
    ...
    for j=1:numel(QC(i).restruns)
        fprintf(fid,'%d',QC(i).restruns(j));
        if j~=numel(QC(i).restruns)
            fprintf(fid,' ');
        end
    end
    fprintf(fid,')\n');
...
for j=1:size(bolds,1)
    fcimg(:,trborders(j,1):trborders(j,2))=read_4dfpimg(bolds{j,1});
end
...
QC(i).MEAN_CSF(:,stage)=mean(tempimg(QC(i).CSFMASK,:))';
QC(i).MEAN_WM(:,stage)=mean(tempimg(QC(i).WMMASK,:))';
QC(i).MEAN_GLM(:,stage)=mean(tempimg(QC(i).GLMMASK,:))';

QC(i).SD_CSF(:,stage)=std(tempimg(QC(i).CSFMASK,:))';
QC(i).SD_WM(:,stage)=std(tempimg(QC(i).WMMASK,:))';
QC(i).SD_GLM(:,stage)=std(tempimg(QC(i).GLMMASK,:))';
...
[jnk tempDV]=array_calc_rms(difftempimg(QC(i).CSFMASK,:));
tempDV(QC(i).runborders(:,2))=NaN;
QC(i).DV_CSF(:,stage)=tempDV';
[jnk tempDV]=array_calc_rms(difftempimg(QC(i).WMMASK,:));
tempDV(QC(i).runborders(:,2))=NaN;
QC(i).DV_WM(:,stage)=tempDV';
[jnk tempDV]=array_calc_rms(difftempimg(QC(i).GLMMASK,:));
tempDV(QC(i).runborders(:,2))=NaN;
...
if switches.regressiontype==1 | switches.regressiontype==9
    QC(i).MEAN_GM(:,stage)=mean(tempimg(QC(i).GMMASK,:))';
    QC(i).MEAN_WB(:,stage)=mean(tempimg(QC(i).WBMASK,:))';
    
    QC(i).SD_GM(:,stage)=std(tempimg(QC(i).GMMASK,:))';
    QC(i).SD_WB(:,stage)=std(tempimg(QC(i).WBMASK,:))';
    
    QC(i).SDbar_GM(:,stage)=mean(QC(i).SD_GM(:,stage));
    QC(i).SDbar_WB(:,stage)=mean(QC(i).SD_WB(:,stage));
    
    [jnk tempDV]=array_calc_rms(difftempimg(QC(i).GMMASK,:));
    tempDV(QC(i).runborders(:,2))=NaN;
    QC(i).DV_GM(:,stage)=tempDV';
    [jnk tempDV]=array_calc_rms(difftempimg(QC(i).WBMASK,:));
    tempDV(QC(i).runborders(:,2))=NaN;
