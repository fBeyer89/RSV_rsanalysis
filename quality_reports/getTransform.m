function [H] = getTransform(t,h,TH,Tr,ofac,hifac)

%function [H,f,s,c,tau,w] = getTransform(t,h,TH,Tr,ofac,hifac)

%input: TRtimes(nonscrubvols),temprun(:,voxbin(v):voxbin(v+1)),TRtimes,TR,ofac,hifac
...
% added a few comments: Michael Gaebler, 2.2.17

N = size(h,1); %Number of time points
T = max(t) - min(t); %Total time span (in seconds?)
